...
|---|---|---|---|
| ① | **CLIP 으로 yes/no 직접 <br>판정** | "Is there X?" 패턴이 들어오면 CLIP 으로 "X 있는 사진" / "X 없는 사진" 임베딩과 이미지 유사도를 비교해 답 결정 | **5 / 5** (yes/no) |
| ② | **CLIP 으로 색상 <br>분류** | "What color..." 패턴이면 12개 색상 단어와 이미지를 매칭해 가장 가까운 색 응답 | **3 / 3** (색상) |
