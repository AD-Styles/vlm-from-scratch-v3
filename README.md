...
> POPE 의 +20%p 는 평가용 60 샘플 안에서 best threshold 를 찾아 측정한 값이라 일반화 보장이 약합니다 (test set 안에서의 hyperparameter tuning). 라이브 데모는 untuned `threshold=0.0` 으로 +3%p 입니다. 자세한 내용은 [📊 정량 측정](#-정량-측정) 참조.
...
1. [🧩 추론 wrapper — 5가지 기법 + 기여 분리](#-추론-wrapper--5가지-기법--기여-분리)
2. [▶️ 직접 확인하기 (Live Demo · 테스트 이미지 · UI 검증)](#%EF%B8%8F-직접-확인하기)
3. [📊 정량 측정 (POPE · VQAv2 · latency)](#-정량-측정)
4. [🔄 v3 가 v2 대비 무엇이 바뀌었나](#-v3-가-v2-대비-무엇이-바뀌었나)
5. [🏗️ 모델 구조 + 0.5B 선택 이유](#%EF%B8%8F-모델-구조)
6. [🇰🇷 Step 1 — 한국어 데이터 추가](#-step-1--한국어-데이터-추가-catastrophic-forgetting-해소)
7. [🧪 Step 2 — ViT-L/14 시도 (효과 없어서 채택 X)](#-step-2--vit-l14-시도-효과-없어서-채택-x)
8. [🛡️ Step 3 — OOD 감지](#%EF%B8%8F-step-3--ood-감지-구현--2케이스-sanity-본격-검증은-v4)
9. [🪶 Step 4 — Slim adapter (1045 MB → 8.28 MB)](#-step-4--slim-adapter-1045-mb--828-mb-출력-변화-없음)
10. [💭 회고](#-회고--v3-작업-과정에서-얻은-것)
11. [⚠️ 한계](#%EF%B8%8F-한계-정직하게-명시)
12. [📚 참고 자료](#-참고-자료)

---

## 🧩 추론 wrapper — 5가지 기법 + 기여 분리
...
> 크기를 줄였다고 모델이 더 똑똑해진 건 아닙니다. PEFT 가 1045 MB 로 저장하던 걸 8.28 MB 로 효율화한 것뿐입니다. 다운로드 시간 / hosting 비용이 줄어들 뿐, 정확도는 변화 없습니다. (Slim 의 본질은 [Step 4 끝부분](#-step-4--slim-adapter-1045-mb--828-mb-출력-변화-없음) 에 따로 적었습니다.)
