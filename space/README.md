...
| OOD 신호 | ❌ 무조건 답변 (hallucination) | ✅ **"모름" layer 추가** (CLIP+entropy, 검증 N=2 — 본격 ROC 분석은 v4) |
...
- **영문 VQA head-to-head**: v3 baseline 36.67% vs v2 34.67% (+2.00%p) — VQAv2 50 samples, greedy decoding 기준 (절대치는 0.5B LLM 한계로 낮음). 추론 wrapper 추가도 자유 서술형 점수에는 영향 없음 — wrapper 의 의미 있는 개선은 POPE 환각 차단 쪽.
